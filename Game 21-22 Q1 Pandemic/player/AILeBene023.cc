...
struct PLAYER_NAME : public Player {
   ...
   */
   ...
   //Moves toward an enemy in the nearest cells, and returns true if so
   bool moved_to_enemy(Unit u) {
    ...
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id)) {
      Unit enemy = unit(c.unit_id);
      if (40 > u.health) move(u.id, BOTTOM);
      else move(u.id, TOP);
      return true;
    }
    //Right
    ...
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id)) {
      Unit enemy = unit(c.unit_id);
      if (40 > u.health) move(u.id, RIGHT);
      else move(u.id, LEFT);
      return true;
    }
     return false;
