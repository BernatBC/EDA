...
struct PLAYER_NAME : public Player {
   ...
   */
     ...
     while (!to_visit.empty()) {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (p != t) {
           ...
           if (terrain[t.i][t.j].type == PATH and path_owner(cell_to_check.path_id) != me()) return direction[t];
           if (terrain[t.i][t.j].type == CITY and city_owner(cell_to_check.city_id) != me()) return direction[t];
         }
   ...
   bool moved_to_enemy(Unit u) {
    ...
    if (p_bottom >= p_top and p_bottom >= p_right and p_bottom >= p_left) {
      move(u.id, BOTTOM);
      return true;
    }
    else if (p_bottom <= p_top and p_top >= p_right and p_top >= p_left) {
      move(u.id, TOP);
      return true;
    }
    else if (p_right >= p_top and p_bottom <= p_right and p_right >= p_left) {
      move(u.id, RIGHT);
      return true;
    }
    else if (p_left >= p_top and p_left >= p_right and p_bottom <= p_left) {
      move(u.id, LEFT);
      return true;
    }
   }
