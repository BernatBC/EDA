     ...
     while (!to_visit.empty()) {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (p != t) {
           ...
           if (cell_to_check.virus > 0) direction_count[direction[t]] -= 1;
           if (cell_to_check.unit_id != -1 and not Unit_is_mine(cell_to_check.unit_id)) direction_count[direction[t]] += 10;
           if ((terrain[t.i][t.j].type == PATH and path_owner(cell_to_check.path_id) != me()) or (terrain[t.i][t.j].type == CITY and city_owner(cell_to_check.city_id) != me())) {
             min_dist = distance[t];
             direction_count[direction[t]] += 50;
           }
         }
