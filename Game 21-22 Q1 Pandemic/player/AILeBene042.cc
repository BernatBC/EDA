     ...
     while (!to_visit.empty()) {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (p != t) {
           ...
           direction_count[direction[t]] -= cell_to_check.virus ;
           if (cell_to_check.unit_id != -1 and not Unit_is_mine(cell_to_check.unit_id)) direction_count[direction[t]] += 40;
           if ((terrain[t.i][t.j].type == PATH and path_owner(cell_to_check.path_id) != me()) or (terrain[t.i][t.j].type == CITY and city_owner(cell_to_check.city_id) != me())) {
             min_dist = distance[t];
             direction_count[direction[t]] += 160;
           }
         }
