     ...
     while (!to_visit.empty()) {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (p != t) {
           ...
           if (not infected) direction_count[direction[t]] -= cell_to_check.virus ;
           if (cell_to_check.unit_id != -1 and not Unit_is_mine(cell_to_check.unit_id)) direction_count[direction[t]] += 100;
           if ((terrain[t.i][t.j].type == PATH and path_owner(cell_to_check.path_id) != me()) or (terrain[t.i][t.j].type == CITY and city_owner(cell_to_check.city_id) != me())) {
             min_dist = distance[t];
             direction_count[direction[t]] += 500;
           }
         }
  ...
  virtual void play () {
    ...
    for (int i = 0; i < units.size(); ++i) {
      ...
      if (not moved_to_enemy(u)) {
        ...
        bool infected = u.damage > 0;
        move(units[i], bfs(u.pos, infected));
      }
    }
  }
