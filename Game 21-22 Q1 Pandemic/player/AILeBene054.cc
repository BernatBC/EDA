         ...
         if (p != t) {
           ...
           if (terrain[t.i][t.j].type == PATH and path_owner(cell_to_check.path_id) != me()) {
             ...
             direction_count[direction[t]] += 1;
              if (path_owner(cell_to_check.path_id) == -1) direction_count[direction[t]] += 2;
           }
           else if (terrain[t.i][t.j].type == CITY and city_owner(cell_to_check.city_id) != me()) {
             min_dist = distance[t];
             direction_count[direction[t]] += 1;
             if (city_owner(cell_to_check.city_id) == -1) direction_count[direction[t]] += 2;
           }
         }
