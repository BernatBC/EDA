...
struct PLAYER_NAME : public Player {
   ...
   */
   ...
   void Get_map() {
     ...
     for (int i = 0; i < nb_cities(); ++i) cities[i] = 0;
     //Initialize path size
     for (int i = 0; i < nb_paths(); ++i) paths[i] = 0;
     //Get map
     for (int i = 0; i < rows(); ++i) {
       for (int j = 0; j < cols(); ++j) {
         terrain[i][j] = cell(i, j);
         //Add city size
         if (terrain[i][j].city_id != -1) cities[terrain[i][j].city_id]++;
         //Add path size
         if (terrain[i][j].path_id != -1) paths[terrain[i][j].path_id]++;
       }
     }
   }
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (p != t) {
           ...
           if (terrain[t.i][t.j].type == PATH and path_owner(cell_to_check.path_id) != me()) {
             min_dist = distance[t];
             direction_count[direction[t]] += 1;
              if (path_owner(cell_to_check.path_id) == -1) direction_count[direction[t]] += 5;
           }
           else if (terrain[t.i][t.j].type == CITY and city_owner(cell_to_check.city_id) != me()) {
             min_dist = distance[t];
             direction_count[direction[t]] += 1;
             if (city_owner(cell_to_check.city_id) == -1) direction_count[direction[t]] += 5;
           }
         }
