   ...
   int compute_score(int city_id) {
     ...
     while (not cities_to_visit.empty()) {
       ...
       for (int i = 1; i < cities[city_to_treat].size(); ++i) {
         ...
         if (path_owner(path_id[make_pair(city_to_treat, cities[city_to_treat][i])]) == me()) {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (p != t) {
           ...
           if (terrain[t.i][t.j].type == PATH and path_owner(cell_to_check.path_id) != me()) {
             ...
             min_dist = distance[t];
             direction_count[direction[t]] += compute_score(cell_to_check.city_id);
           }
         }
