...
struct PLAYER_NAME : public Player {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (p != t) {
           ...
           if (terrain[t.i][t.j].type == PATH and path_owner(cell_to_check.path_id) != me()) {
             ...
             if (city_owner(city1) == me() and city_owner(city2) == me()) direction_count[direction[t]] = max(compute_score(city1, cell_to_check.path_id), direction_count[direction[t]]);
             else direction_count[direction[t]] += 1;
           }
           else if (terrain[t.i][t.j].type == CITY and city_owner(cell_to_check.city_id) != me()) {
             min_dist = distance[t];
             direction_count[direction[t]] = max(compute_score(cell_to_check.city_id, -1), direction_count[direction[t]]);
           }
         }
  ...
  virtual void play () {
    ...
    for (int i = 0; i < units.size(); ++i) {
      ...
      if (d == NONE) {
        //Get best direction
        if (status(me()) > 0.9) d = bfs_low_cpu(u.pos);
        else d = bfs(u.pos);
      }
      move(units[i], d);
    }
