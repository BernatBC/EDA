...
struct PLAYER_NAME : public Player {
   ...
   */
   ...
   Dir enemy_position(City city_p, Pos p) {
     for (int k = 0; k < city_p.size(); ++k) {
       Cell cell_p = cell(city_p[k]);
       if (cell_p.unit_id != -1 and not Unit_is_mine(cell_p.unit_id)) {
         if (p.i > city_p[k].i) return TOP;
         else if (p.i > city_p[k].i) return BOTTOM;
         else if (p.j > city_p[k].j) return LEFT;
         else return RIGHT;
       }
     }
   }
   ...
   Dir bfs(Pos p) {
     //If I'm in a city conquered by anyone else, go towards the enemy
     Cell cell_p = cell(p);
     if (cell_p.city_id != -1 and city_owner(cell_p.city_id) != me()) {
       City city_p = city(cell_p.city_id);
       return enemy_position(city_p, p);
     }
     list<Pos> to_visit;
     set<Pos> visited;
