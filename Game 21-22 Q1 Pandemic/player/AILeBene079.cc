   ...
   Dir bfs(Pos p) {
     ...
     while (!to_visit.empty()) {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (p.i > rows()/2) direction_count[TOP] *= 3;
         else direction_count[BOTTOM] *= 3;
         if (p.j > cols()/2) direction_count[LEFT] *= 3;
         else direction_count[RIGHT] *= 3;
