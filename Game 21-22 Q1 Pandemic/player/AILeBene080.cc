   ...
   Dir bfs(Pos p) {
     ...
     while (!to_visit.empty()) {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (p.i > rows()/2) direction_count[TOP] *= 5;
         else direction_count[BOTTOM] *= 5;
         if (p.j > cols()/2) direction_count[LEFT] *= 5;
         else direction_count[RIGHT] *= 5;
