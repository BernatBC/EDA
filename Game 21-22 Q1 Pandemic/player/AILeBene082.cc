   ...
   Dir bfs(Pos p) {
     ...
     while (!to_visit.empty()) {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (p.i > rows()/2 + 20) direction_count[TOP] *= 2;
         else if (p.i < rows()/2 - 20) direction_count[BOTTOM] *= 2;
         if (p.j > cols()/2 + 20) direction_count[LEFT] *= 2;
         else if (p.j < cols()/2 - 20) direction_count[RIGHT] *= 2;
