     ...
     while (!to_visit.empty()) {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (min_dist != 0 and min_dist < distance[t]) {
           ...
           if (p.i > rows()/2) direction_count[TOP] *= 4;
           else direction_count[BOTTOM] *= 4;
           if (p.j > cols()/2) direction_count[LEFT] *= 4;
           else direction_count[RIGHT] *= 4;
