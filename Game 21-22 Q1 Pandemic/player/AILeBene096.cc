     ...
     while (!to_visit.empty()) {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (min_dist != 0 and min_dist < distance[t]) {
           ...
           if (p.i > rows()/2 + 5) direction_count[TOP] *= 3;
           else if (p.i < rows()/2 - 5) direction_count[BOTTOM] *= 3;
           if (p.j > cols()/2 + 5) direction_count[LEFT] *= 3;
           else if (p.j < cols()/2 - 5) direction_count[RIGHT] *= 3;
