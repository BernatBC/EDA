     ...
     while (!to_visit.empty()) {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (min_dist != 0 and min_dist < distance[t]) {
           ...
           if (p.i + p.j > (cols() + rows())/2) {
             if (p.i > p.j) direction_count[TOP] *= 2;
             else direction_count[LEFT] *= 2;
           }
           else {
             if (p.i > p.j) direction_count[RIGHT] *= 2;
             else direction_count[BOTTOM] *= 2;
           }

           if (p.i > rows()/2 + 15) direction_count[TOP] *= 2;
           else if (p.i < rows()/2 - 15) direction_count[BOTTOM] *= 2;
           if (p.j > cols()/2 + 15) direction_count[LEFT] *= 2;
           else if (p.j < cols()/2 - 15) direction_count[RIGHT] *= 2;
