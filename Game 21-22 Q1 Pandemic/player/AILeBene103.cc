...
struct PLAYER_NAME : public Player {
   ...
   */
     ...
     while (!to_visit.empty()) {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (min_dist != 0 and min_dist < distance[t]) {
           ...
           //Remove option if enemy is not worth
           if (e_top != 0) direction_count[TOP] = 0;
           if (e_bottom != 0) direction_count[BOTTOM] = 0;
           if (e_left != 0) direction_count[LEFT] = 0;
           if (e_right != 0) direction_count[RIGHT] = 0;
   ...
   Dir moved_to_enemy(Unit u) {
    ...
    if (c.unit_id != -1) {
      ...
      if (Unit_is_mine(c.unit_id) or (u.health < 25 and e.health > 40)) e_bottom = 1;
      else if (not Unit_is_mine(c.unit_id)) {
        ++p_bottom;
        if (c.city_id != -1) {
          ++p_bottom;
    ...
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id)) {
      Unit e = unit(c.unit_id);
      if (u.health < 25 and e.health > 40 or (u.health < 25 and e.health > 40)) e_top = 1;
      else if (not Unit_is_mine(c.unit_id)) {
        ++p_top;
        if (c.city_id != -1) {
          ++p_top;
    ...
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id)) {
      Unit e = unit(c.unit_id);
      if (u.health < 25 and e.health > 40 or (u.health < 25 and e.health > 40)) e_right = 1;
      else if (not Unit_is_mine(c.unit_id)) {
        ++p_right;
        if (c.city_id != -1) {
          ++p_right;
    ...
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id)) {
      Unit e = unit(c.unit_id);
      if (u.health < 25 and e.health > 40 or (u.health < 25 and e.health > 40)) e_left = 1;
      else if (not Unit_is_mine(c.unit_id)) {
        ++p_left;
        if (c.city_id != -1) {
          ++p_left;
  ...
  virtual void play () {
    ...
    units = my_units(me());
    for (int i = 0; i < units.size(); ++i) {
      //Reset enemies not worth fighting
      e_top = e_bottom = e_left = e_right = 0;
      Unit u = unit(units[i]);
      //Check if can attack
      Dir d = moved_to_enemy(u);
      if (d == NONE) {
        //Get best direction
        if (status(me()) > 0.8) d = bfs_low_cpu(u.pos);
        else d = bfs(u.pos);
      }
      move(units[i], d);
    }
