...
struct PLAYER_NAME : public Player {
   ...
   */
   ...
   //Returns number of enemies in certain city
   int num_enemies(City city_to_check) {
     int n_enemies = 0;
     for (int i = 0; i < city_to_check.size(); ++i) {
       Cell k = cell(city_to_check[i]);
       if (k.unit_id != -1 and not Unit_is_mine(k.unit_id)) ++n_enemies;
     }
     return n_enemies;
   }
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (p != t) {
           ...
           if (terrain[t.i][t.j].type == PATH and path_owner(cell_to_check.path_id) != me()) {
             ...
             min_dist = distance[t];
             City city_to_check = city(cell_to_check.city_id);
             direction_count[direction[t]] += compute_score(cell_to_check.city_id, -1)/(num_enemies(city_to_check) + 1);
           }
         }
