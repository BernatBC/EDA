...
struct PLAYER_NAME : public Player {
   ...
   */
     ...
     while (!to_visit.empty()) {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (min_dist != 0 and min_dist < distance[t]) {
           ...
           if (e_top) direction_count[TOP] = 0;
           if (e_bottom) direction_count[BOTTOM] = 0;
           if (e_left) direction_count[LEFT] = 0;
           if (e_right) direction_count[RIGHT] = 0;
   ...
   Dir moved_to_enemy(Unit u) {
    ...
    else if (u.damage > 0 and c.mask) e_bottom = true;
    //Top
    p.i = u.pos.i - 1;
    c = cell(p);
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id)) {
    ...
    else if (u.damage > 0 and c.mask) e_top = true;
    //Right
    p.i = u.pos.i;
    p.j = u.pos.j + 1;
    c = cell(p);
    ...
    else if (u.damage > 0 and c.mask) e_right = true;
    //Left
    p.j = u.pos.j - 1;
    c = cell(p);
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id)) {
    ...
    else if (u.damage > 0 and c.mask) e_left = true;
