     ...
     while (!to_visit.empty()) {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (p != t) {
           ...
           //If I have an infected enemy closer than 3 cells, attack
           if (cell_to_check.unit_id != -1 and not Unit_is_mine(cell_to_check.unit_id) and unit(cell_to_check.unit_id).damage > 0 and distance[t] <= 2) return direction[t];
         }
