   ...
   */
   ...
   Dir moved_to_enemy(Unit u) {
    ...
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id)) {
      ++p_bottom;
      if (unit(c.unit_id).health < 25) p_bottom += 3;
      if (c.city_id != -1) {
        ++p_bottom;
    ...
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id)) {
      ++p_top;
      if (unit(c.unit_id).health < 25) p_top += 3;
      if (c.city_id != -1) {
        ++p_top;
    ...
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id)) {
      ++p_right;
      if (unit(c.unit_id).health < 25) p_right += 3;
      if (c.city_id != -1) {
        ++p_right;
    ...
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id)) {
      ++p_left;
      if (unit(c.unit_id).health < 25) p_left += 3;
      if (c.city_id != -1) {
        ++p_left;
