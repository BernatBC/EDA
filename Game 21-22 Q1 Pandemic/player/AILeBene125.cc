     ...
     while (!to_visit.empty()) {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (p != t) {
           ...
           if (cell_to_check.unit_id != -1 and not Unit_is_mine(cell_to_check.unit_id) and (unit(cell_to_check.unit_id).health <= 40 or unit(my_id).health - unit(cell_to_check.unit_id).health >= 32)) return direction[t];
         }
