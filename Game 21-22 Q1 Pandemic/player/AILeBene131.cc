...
struct PLAYER_NAME : public Player {
   ...
   */
     ...
     while (!to_visit.empty()) {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (p != t) {
           ...
           if (round() < 170 and cell_to_check.unit_id != -1 and not Unit_is_mine(cell_to_check.unit_id) and (unit(cell_to_check.unit_id).health <= 40 or unit(my_id).health - unit(cell_to_check.unit_id).health >= 40)) return direction[t];
         }
   ...
   //Simplified bfs to retain cities and paths
   Dir bfs_retain(Pos p) {
     list<Pos> to_visit;
     set<Pos> visited;
     map<Pos,Dir>direction;
     to_visit.push_back(p);
     ...
     while (!to_visit.empty()) {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (p != t) {
           Cell cell_to_check = cell(t);
           if (terrain[t.i][t.j].type == PATH or terrain[t.i][t.j].type == CITY ) return direction[t];
         }
         //If grass adds adjacent positions to check
         if (terrain[t.i][t.j].type != WALL) {
           Pos c;
  ...
  virtual void play () {
    ...
    for (int i = 0; i < units.size(); ++i) {
      ...
      if (d == NONE) {
        //Get best direction
        if (round() > 190) d = bfs_retain(u.pos);
        else if (status(me()) > 0.75) d = bfs_low_cpu(u.pos);
