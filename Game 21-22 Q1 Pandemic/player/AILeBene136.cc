...
struct PLAYER_NAME : public Player {
   ...
   */
     ...
     while (!to_visit.empty()) {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (min_dist != 0 and min_dist < distance[t]) {
           //Clear score for unaviable cells
           if (not top_aviable) direction_count[TOP] = 0;
           else if (not bottom_aviable) direction_count[BOTTOM] = 0;
           else if (not left_aviable) direction_count[LEFT] = 0;
           else if (not right_aviable) direction_count[RIGHT] = 0;
   ...
   Dir moved_to_enemy(Unit u) {
    ...
    if (p.i < 70) {
      c = cell(p);
      if (c.unit_id != -1 and not Unit_is_mine(c.unit_id) and unit(c.unit_id).health > 40) bottom_aviable = false;
    }
    //Top
    p.i = u.pos.i - 1;
    c = cell(p);
    ...
    if (p.i >= 0) {
      c = cell(p);
      if (c.unit_id != -1 and not Unit_is_mine(c.unit_id) and unit(c.unit_id).health > 40)  top_aviable = false;
    }
    //Right
    p.i = u.pos.i;
    p.j = u.pos.j + 1;
    ...
    if (p.j < 70) {
      c = cell(p);
      if (c.unit_id != -1 and not Unit_is_mine(c.unit_id) and unit(c.unit_id).health > 40)  right_aviable = false;
    }
    //Left
    p.j = u.pos.j - 1;
    c = cell(p);
    ...
    if (p.j >= 0) {
      c = cell(p);
      if (c.unit_id != -1 and not Unit_is_mine(c.unit_id) and unit(c.unit_id).health > 40)  left_aviable = false;
    }
    //Left-top
    p.i = u.pos.i - 1;
    p.j = u.pos.j - 1;
   ...
   */
  virtual void play () {
    top_aviable = bottom_aviable = left_aviable = right_aviable = true;
