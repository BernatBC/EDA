...
struct PLAYER_NAME : public Player {
   ...
   */
     ...
     while (!to_visit.empty()) {
       ...
       if (visited.find(t) == visited.end()) {
         ...
         if (p != t) {
           ...
           if (cell_to_check.unit_id != -1 and not Unit_is_mine(cell_to_check.unit_id) and (unit(cell_to_check.unit_id).health <= 40 or unit(my_id).health - unit(cell_to_check.unit_id).health >= 40)) return direction[t];
         }
   ...
   Dir moved_to_enemy(Unit u) {
    ...
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id)) {
      ++p_bottom;
      if (unit(c.unit_id).health <= 40) p_bottom += 5;
      if (c.city_id != -1) {
        ++p_bottom;
    ...
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id)) {
      ++p_top;
      if (unit(c.unit_id).health <= 40) p_top += 5;
      if (c.city_id != -1) {
        ++p_top;
    ...
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id)) {
      ++p_right;
      if (unit(c.unit_id).health <= 40) p_right += 5;
      if (c.city_id != -1) {
        ++p_right;
    ...
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id)) {
      ++p_left;
      if (unit(c.unit_id).health <= 40) p_left += 5;
      if (c.city_id != -1) {
        ++p_left;
   ...
   Dir moved_to_enemy_protect(Unit u) {
    Pos p = u.pos;
    Cell c;
    int p_bottom = 0;
    int p_top = 0;
    ...
    p.i = u.pos.i + 2;
    c = cell(p);
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id) and unit(c.unit_id).health > 40) p_top += 5;
    //Top
    p.i = u.pos.i - 1;
    c = cell(p);
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id)) {
    ...
    p.i = u.pos.i - 2;
    c = cell(p);
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id) and unit(c.unit_id).health > 40) p_bottom += 5;
    //Right
    p.i = u.pos.i;
    p.j = u.pos.j + 1;
    c = cell(p);
    ...
    p.j = u.pos.j + 2;
    c = cell(p);
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id) and unit(c.unit_id).health > 40) p_left += 5;
    //Left
    p.j = u.pos.j - 1;
    c = cell(p);
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id)) {
    ...
    p.j = u.pos.j - 2;
    c = cell(p);
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id) and unit(c.unit_id).health > 40) p_right += 5;
    //Left-top
    p.i = u.pos.i - 1;
    p.j = u.pos.j - 1;
    c = cell(p);
    if (c.unit_id != -1 and not Unit_is_mine(c.unit_id) and unit(c.unit_id).health > 40) {
      p_right += 5;
      p_bottom += 5;
    }
    //Right-top
   ...
   //Check if it's safe to leave the city
   bool safe_to_leave(int city_id) {
     City c = city(city_id);
     int min_i = 69;
     int min_j = 69;
     int max_i = 0;
     ...
     for (int i = 0; i < c.size(); ++i) {
       if (c[i].i > max_i) max_i = c[i].i;
       if (c[i].i < min_i) min_i = c[i].i;
       if (c[i].j > max_j) max_j = c[i].j;
       if (c[i].j < min_j) min_j = c[i].j;
       Cell k = cell(c[i]);
       if (k.unit_id != -1 and not Unit_is_mine(k.unit_id)) ++enemies;
       if (k.unit_id != -1 and Unit_is_mine(k.unit_id)) ++my_units;
     }
     //Surroundings
  ...
  virtual void play () {
    ...
    for (int i = 0; i < units.size(); ++i) {
      ...
      //Check if can attack
      Dir d;
      if (round() < 20) moved_to_enemy_protect(u);
      d = moved_to_enemy(u);
      if (d == NONE) {
        //Get best direction
        if (status(me()) > 0.6) d = bfs_low_cpu(u.pos);
        else {
          d = bfs(u.pos, u.id);
          //Check if city is safe to leave
          Cell k = cell(u.pos);
          if (k.city_id != -1 and city_owner(k.city_id) == me()) {
            Pos p = u.pos;
            if (d == TOP) p.i--;
            else if (d == BOTTOM) p.i++;
            else if (d == LEFT) p.j--;
            else if (d == RIGHT) p.j++;
            Cell k2 = cell(p);
            if (k2.city_id == -1 and not safe_to_leave(k.city_id)) d = NONE;
          }
        }
      }
      move(units[i], d);
